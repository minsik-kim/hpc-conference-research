...
| DEFT: Joint Task Placement and DVFS for Energy-Efficient Multi-GPU Runtimes | ICS 2026, session S22 Energy-Aware Systems (J. Chen, M. Pericàs) | `CLOSED_ACCESS` (effectively) — census `NOT_SEARCHED` | `TITLE_AND_CENSUS_ROW` | **None** — placement plus frequency control | **`UNRESOLVED`** — DVFS on a GPU is a real device mechanism, but joint placement+DVFS is a classic energy-scheduling formulation | `UNRESOLVED` | **ROW OWNED BY THE POWER CLUSTER — overlap noted only.** The overlap with this cluster is real and worth stating: `GPU-ISC26-165` finds that **power throttling is the interference channel MIG does not isolate**, so a multi-GPU runtime that controls DVFS is manipulating exactly the resource that defeats hardware partitioning. That connection should be carried into the power cluster's reading. |
